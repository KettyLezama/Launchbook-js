...
let firstAd = document.getElementById('sidebar-right').getElementsByClassName('ads')[0].getElementsByClassName('ad-slot')[0];
...
let thirdAd = document.getElementById('sidebar-right').getElementsByClassName('ads')[0].getElementsByClassName('ad-slot')[2];
thirdAd.getElementsByTagName('img')[0].setAttribute('src', 'http://placebear.com/200/300');

// Exercise 8): Find Sam's post and change its text to something incredible.
let samPostParagraph = document.getElementsByClassName('media-body')[4].getElementsByTagName('p')[0];
samPostParagraph.innerText = '"Something incredible."'
...
let secondPostActions = document.getElementsByClassName('inline-list post-actions')[1];
let shareLink = secondPostActions.getElementsByTagName('li')[2];
shareLink.classList.add('post-shared');

// Exercise 11: On the second post, remove the `.post-shared` class from the `li` containing the text Share.
shareLink.classList.remove('post-shared');